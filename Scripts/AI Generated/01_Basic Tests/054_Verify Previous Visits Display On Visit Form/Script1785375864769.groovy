...
String firstName = 'PreviousFirst' + stamp
String lastName = 'PreviousLast' + stamp
String address = stamp + ' Previous Visit Street'
String city = 'PreviousCity'
String telephone = stamp.substring(stamp.length() - 10)
String petName = 'PreviousPet' + stamp
String visitDescription = 'Previous visit form check ' + stamp
...
visitDateInput.addProperty('xpath', ConditionType.EQUALS,
    "//*[@id='date' and @name='date' and @type='date']")
...
TestObject previousVisitsHeading = new TestObject('previousVisitsHeading')
previousVisitsHeading.addProperty('xpath', ConditionType.EQUALS,
    "//*[self::b or self::h2 or self::h3][normalize-space(.)='Previous Visits']")
...
""", null)
    ...
    String visitDate = WebUI.getAttribute(visitDateInput, 'value')

    WebUI.verifyMatch(visitDate, '\\d{4}-\\d{2}-\\d{2}', true)
    ...
    WebUI.waitForElementVisible(previousVisitsHeading, 10)

    TestObject previousVisitRow = new TestObject('previousVisitRow')
    previousVisitRow.addProperty('xpath', ConditionType.EQUALS,
        "//*[self::b or self::h2 or self::h3][normalize-space(.)='Previous Visits']/following::table[1]//tr[" +
        "td[1][normalize-space(.)='" + visitDate + "'] and td[2][normalize-space(.)='" + visitDescription + "']]")

    TestObject previousVisitDate = new TestObject('previousVisitDate')
    previousVisitDate.addProperty('xpath', ConditionType.EQUALS,
        "//*[self::b or self::h2 or self::h3][normalize-space(.)='Previous Visits']/following::table[1]//tr[" +
        "td[1][normalize-space(.)='" + visitDate + "'] and td[2][normalize-space(.)='" + visitDescription + "']]/td[1]")

    TestObject previousVisitDescription = new TestObject('previousVisitDescription')
    previousVisitDescription.addProperty('xpath', ConditionType.EQUALS,
        "//*[self::b or self::h2 or self::h3][normalize-space(.)='Previous Visits']/following::table[1]//tr[" +
        "td[1][normalize-space(.)='" + visitDate + "'] and td[2][normalize-space(.)='" + visitDescription + "']]/td[2]")

    WebUI.verifyElementVisible(previousVisitRow)

    WebUI.verifyElementText(previousVisitDate, visitDate)

    WebUI.verifyElementText(previousVisitDescription, visitDescription)
} finally {
    WebUI.closeBrowser()
}
