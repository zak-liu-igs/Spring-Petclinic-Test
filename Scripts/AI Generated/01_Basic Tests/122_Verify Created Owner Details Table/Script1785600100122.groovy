...
String firstName = 'Details' + token.substring(token.length() - 5)
String lastName = 'Table' + token.substring(token.length() - 7)
String address = '122 Details Boulevard'
...
try {
    ...
    WebUI.verifyElementText(detailCell('Address'), address)
    WebUI.verifyElementText(detailCell('City'), city)
    WebUI.verifyElementText(detailCell('Telephone'), telephone)
} finally {
    WebUI.closeBrowser()
}
