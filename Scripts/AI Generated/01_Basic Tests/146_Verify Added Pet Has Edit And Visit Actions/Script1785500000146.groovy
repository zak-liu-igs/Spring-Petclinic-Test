...
""", null)
    ...
    TestObject editLink = xpath('pet edit action',
        "//a[normalize-space(.)='Edit Pet' and starts-with(@href,'/owners/" + ownerId + "/pets/') and substring(@href,string-length(@href)-4)='/edit']")
    TestObject visitLink = xpath('pet visit action',
        "//a[normalize-space(.)='Add Visit' and starts-with(@href,'/owners/" + ownerId + "/pets/') and substring(@href,string-length(@href)-10)='/visits/new']")
