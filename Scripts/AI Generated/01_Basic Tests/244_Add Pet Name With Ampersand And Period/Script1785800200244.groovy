...
String petName = 'Rock & Roll.' + token.substring(token.length() - 6)
...
try {
    ...
    WebUI.setText(findTestObject(repository + 'input_First Name'), 'Punctuation')
    WebUI.setText(findTestObject(repository + 'input_lastName'), 'Owner' + token.substring(token.length() - 7))
    WebUI.setText(findTestObject(repository + 'input_Address'), '244 Ampersand Avenue')
    ...
    TestObject punctuatedHamsterDetails = xpath('punctuated hamster details',
    ...
    WebUI.verifyElementPresent(punctuatedHamsterDetails, 10)
