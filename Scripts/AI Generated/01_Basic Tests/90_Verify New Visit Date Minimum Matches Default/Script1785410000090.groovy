...
String firstName = 'MinF' + stamp
String lastName = 'MinL' + stamp
String address = stamp + ' Minimum Date Street'
String city = 'MinCity'
String telephone = stamp.substring(stamp.length() - 10)
String petName = 'MinPet' + stamp
...
""", null)
    ...
    String defaultDate = WebUI.getAttribute(visitDateInput, 'value')
    String minimumDate = WebUI.getAttribute(visitDateInput, 'min')

    WebUI.verifyEqual(defaultDate, tomorrow)

    WebUI.verifyEqual(minimumDate, tomorrow)

    WebUI.verifyEqual(minimumDate, defaultDate)
} finally {
    WebUI.closeBrowser()
}
