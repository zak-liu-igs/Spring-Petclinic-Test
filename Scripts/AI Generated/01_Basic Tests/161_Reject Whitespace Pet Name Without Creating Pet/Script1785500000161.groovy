...
try {
    ...
    WebUI.setText(findTestObject(repository + 'input_First Name'), 'BlankPet')
    WebUI.setText(findTestObject(repository + 'input_lastName'), 'Owner' + token.substring(token.length() - 7))
    WebUI.setText(findTestObject(repository + 'input_Address'), '161 Blank Avenue')
    ...
    WebUI.setText(findTestObject(repository + 'input_PetName'), '     ')
...
""", null)
    ...
    Number editActionCount = (Number) WebUI.executeJavaScript(
        ...
        null)
    WebUI.verifyEqual(editActionCount.intValue(), 0)
} finally {
    WebUI.closeBrowser()
}
