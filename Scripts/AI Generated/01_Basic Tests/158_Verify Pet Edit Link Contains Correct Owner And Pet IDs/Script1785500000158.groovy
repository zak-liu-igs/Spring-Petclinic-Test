...
String petName = 'EditLink' + token.substring(token.length() - 6)
...
try {
    ...
    WebUI.setText(findTestObject(repository + 'input_First Name'), 'EditLink')
    WebUI.setText(findTestObject(repository + 'input_lastName'), 'Owner' + token.substring(token.length() - 7))
    WebUI.setText(findTestObject(repository + 'input_Address'), '158 Link Avenue')
...
""", null)
    ...
    TestObject editLink = xpath('edit link for owner',
        "//a[normalize-space(.)='Edit Pet' and starts-with(@href,'/owners/" + ownerId + "/pets/')]")
    String href = WebUI.getAttribute(editLink, 'href')
    def hrefMatcher = href =~ /\/owners\/(\d+)\/pets\/(\d+)\/edit$/
    WebUI.verifyEqual(hrefMatcher.find(), true)
    WebUI.verifyEqual(hrefMatcher.group(1), ownerId)
    WebUI.verifyMatch(hrefMatcher.group(2), '\\d+', true)
} finally {
    WebUI.closeBrowser()
}
