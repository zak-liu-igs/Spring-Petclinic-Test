...
try {
    ...
    ownerNames.eachWithIndex { String fullName, int index ->
        ...
        WebUI.waitForPageLoad(10)
        def ownerMatcher = normalizeUrl(WebUI.getUrl()) =~ /\/owners\/(\d+)\/?$/
    ...
    WebUI.verifyMatch(normalizeUrl(WebUI.getUrl()), baseUrl + '/owners/' + ownerIds[0] + '/pets/new/?', true)
    ...
    WebUI.verifyMatch(normalizeUrl(WebUI.getUrl()), baseUrl + '/owners/' + ownerIds[1] + '/pets/new/?', true)
} finally {
    WebUI.closeBrowser()
}
