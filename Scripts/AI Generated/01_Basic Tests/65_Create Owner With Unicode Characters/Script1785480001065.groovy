...
String uniquePart = stamp.substring(stamp.length() - 8)
String firstName = '測試名' + uniquePart
String lastName = '使用者' + uniquePart
String fullName = firstName + ' ' + lastName
String address = '臺北市春日路' + uniquePart + '號'
String city = '臺北市'
String telephone = '2' + stamp.substring(stamp.length() - 9)

TestObject savedUnicodeOwner = new TestObject('savedUnicodeOwner')
savedUnicodeOwner.addProperty('xpath', ConditionType.EQUALS,
...
try {
    ...
    WebUI.waitForElementVisible(savedUnicodeOwner, 10)

    WebUI.verifyElementVisible(savedUnicodeOwner)
} finally {
    WebUI.closeBrowser()
}
