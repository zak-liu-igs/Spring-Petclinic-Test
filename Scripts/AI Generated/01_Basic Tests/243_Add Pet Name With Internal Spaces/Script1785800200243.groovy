...
String petName = 'Blue Sky ' + token.substring(token.length() - 6)
...
try {
    ...
    WebUI.setText(findTestObject(repository + 'input_First Name'), 'InternalSpace')
    WebUI.setText(findTestObject(repository + 'input_lastName'), 'Owner' + token.substring(token.length() - 7))
    WebUI.setText(findTestObject(repository + 'input_Address'), '243 Internal Space Avenue')
    ...
    TestObject spacedBirdPet = xpath('spaced bird pet details',
    ...
    WebUI.verifyElementPresent(spacedBirdPet, 10)
