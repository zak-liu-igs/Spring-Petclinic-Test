...
String firstName = 'EmptyHist' + stamp
String lastName = 'EH' + stamp
String address = stamp + ' Empty History Street'
String city = 'HistoryCity'
String telephone = stamp.substring(stamp.length() - 10)
String petName = 'NoVisit' + stamp
...
TestObject scopedVisitHeader = new TestObject('scopedVisitHeader')
scopedVisitHeader.addProperty('xpath', ConditionType.EQUALS,
    '(' + petRowXPath + ")[1]/td[2]//table//tr[th[normalize-space(.)='Visit Date'] and " +
    "th[normalize-space(.)='Description']]")
...
TestObject scopedVisitDataRows = new TestObject('scopedVisitDataRows')
scopedVisitDataRows.addProperty('xpath', ConditionType.EQUALS,
    '(' + petRowXPath + ')[1]/td[2]//table//tr[td and not(.//a)]')
...
""", null)
    ...
    WebUI.verifyElementVisible(scopedVisitHeader)

    WebUI.verifyElementVisible(scopedAddVisitLink)

    int visitDataRowCount = WebUI.findWebElements(scopedVisitDataRows, 2).size()

    WebUI.verifyEqual(visitDataRowCount, 0)
} finally {
    WebUI.closeBrowser()
}
