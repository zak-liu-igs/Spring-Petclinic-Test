...
try {
    ...
    // PetClinic can append a path parameter like ;jsessionid=... after the owner id.
    // Normalize the URL before validating the stable route.
    String normalizedUrl = WebUI.getUrl().replaceFirst(';jsessionid=[^/?#]+', '')
    WebUI.verifyMatch(normalizedUrl, 'http://localhost:8080/owners/\\d+/?', true)
} finally {
    WebUI.closeBrowser()
}
