...
String firstName = 'EditSpaceF' + stamp
String lastName = 'EditSpaceL' + stamp
String address = stamp + ' Edit Whitespace Street'
String city = 'EditSpaceCity'
String telephone = '9' + stamp.substring(stamp.length() - 9)
String petName = 'KeepPet' + stamp
String whitespaceName = '   '

String savedPetRowXPath = "//h2[normalize-space(.)='Pets and Visits']/following-sibling::table[1]//tr[" +
...
TestObject savedPetRow = new TestObject('savedPetRow')
savedPetRow.addProperty('xpath', ConditionType.EQUALS, savedPetRowXPath)

TestObject scopedEditPetLink = new TestObject('scopedEditPetLink')
scopedEditPetLink.addProperty('xpath', ConditionType.EQUALS,
    '(' + savedPetRowXPath + ")[1]/td[2]//a[normalize-space(.)='Edit Pet']")

TestObject nameRequiredError = new TestObject('nameRequiredError')
nameRequiredError.addProperty('xpath', ConditionType.EQUALS,
    "//form//label[@for='name' and normalize-space(.)='Name']/following-sibling::div[1]" +
    "//span[contains(concat(' ', normalize-space(@class), ' '), ' help-inline ') and normalize-space(.)='is required']")
...
""", null)
    ...
    WebUI.selectOptionByLabel(findTestObject('Page_PetClinic  a Spring Framework demonstration/select_Type'), 'lizard', false)
    ...
    WebUI.setText(findTestObject('Page_PetClinic  a Spring Framework demonstration/input_PetName'), whitespaceName)

    WebUI.click(findTestObject('Page_PetClinic  a Spring Framework demonstration/button_Update Pet'))

    WebUI.waitForElementVisible(nameRequiredError, 10)

    WebUI.verifyElementVisible(nameRequiredError)

    WebUI.verifyElementAttributeValue(findTestObject('Page_PetClinic  a Spring Framework demonstration/input_PetName'),
        'value', whitespaceName, 10)
    ...
    WebUI.waitForElementVisible(savedPetRow, 10)

    WebUI.verifyElementVisible(savedPetRow)
} finally {
    WebUI.closeBrowser()
}
