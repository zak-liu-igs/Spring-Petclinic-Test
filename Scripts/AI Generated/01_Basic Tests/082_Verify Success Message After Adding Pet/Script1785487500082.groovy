...
String firstName = 'PetMsgF' + stamp
String lastName = 'PetMsgL' + stamp
String address = stamp + ' Pet Message Street'
String city = 'PetMessageCity'
String telephone = '4' + stamp.substring(stamp.length() - 9)
String petName = 'MessagePet' + stamp
...
creationSuccessMessage.addProperty('xpath', ConditionType.EQUALS,
    "//div[@id='success-message' and contains(concat(' ', normalize-space(@class), ' '), ' alert-success ')]" +
    "/span[normalize-space(.)='New Pet has been Added']")

TestObject savedPet = new TestObject('savedPet')
savedPet.addProperty('xpath', ConditionType.EQUALS,
...
""", null)
    ...
    WebUI.selectOptionByLabel(findTestObject('Page_PetClinic  a Spring Framework demonstration/select_Type'), 'hamster', false)

    WebUI.click(findTestObject('Page_PetClinic  a Spring Framework demonstration/button_Add Pet'))

    WebUI.waitForElementVisible(creationSuccessMessage, 3)

    WebUI.verifyElementText(creationSuccessMessage, 'New Pet has been Added')
