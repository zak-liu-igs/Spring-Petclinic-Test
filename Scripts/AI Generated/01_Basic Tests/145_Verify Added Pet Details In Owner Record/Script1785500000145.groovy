...
String petName = 'Detail' + token.substring(token.length() - 7)
...
try {
    ...
    WebUI.setText(findTestObject(repository + 'input_First Name'), 'PetDetail')
    WebUI.setText(findTestObject(repository + 'input_lastName'), 'Owner' + token.substring(token.length() - 7))
    WebUI.setText(findTestObject(repository + 'input_Address'), '145 Detail Avenue')
...
""", null)
    ...
    String petDetails = "//dl[.//dd[normalize-space(.)='" + petName +
        "']][.//dd[normalize-space(.)='2024-01-01']][.//dd[normalize-space(.)='lizard']]"
    WebUI.verifyElementPresent(xpath('complete pet details', petDetails), 10)
} finally {
    WebUI.closeBrowser()
}
