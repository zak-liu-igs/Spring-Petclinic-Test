...
String petName = 'Lizard' + token.substring(token.length() - 6)

try {
    WebUI.openBrowser('')
    WebUI.navigateToUrl(baseUrl + '/owners/new')
    WebUI.setText(findTestObject(repository + 'input_First Name'), 'LizardPet')
    WebUI.setText(findTestObject(repository + 'input_lastName'), 'Owner' + token.substring(token.length() - 7))
    WebUI.setText(findTestObject(repository + 'input_Address'), '143 Reptile Avenue')
...
""", null)
    ...
    WebUI.verifyTextPresent('lizard', false)
} finally {
    WebUI.closeBrowser()
}
